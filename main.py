    ...
    def check_prerequisites(self) -> bool:
        ...
        if missing:
            ...
            if "multipass" in missing:
                if is_windows:
                    print("  - Multipass: Téléchargez et installez depuis https://multipass.run/download/windows")
                else:
                    print("  - Multipass: brew install --cask multipass (macOS) ou suivez les instructions sur https://multipass.run/install")

            if "kubectl" in missing:
                if is_windows:
                    print("  - kubectl: Téléchargez et installez depuis https://kubernetes.io/docs/tasks/tools/install-kubectl-windows/")
                    print("    ou utilisez: choco install kubernetes-cli (avec Chocolatey)")
                else:
                    print("  - kubectl: brew install kubectl (macOS) ou suivez les instructions sur https://kubernetes.io/docs/tasks/tools/")
    ...
    def run_command_with_retry(self, node, command, description, max_retries=3):
        ...
        for attempt in range(max_retries):
            try:
                ...
                # Utiliser le shell approprié selon le système d'exploitation
                if platform.system() == "Windows":
                    # Sur Windows, utiliser PowerShell
                    shell_cmd = ["multipass", "exec", node, "--", "powershell", "-Command", command]
                else:
                    # Sur Unix/Linux/macOS, utiliser bash
                    shell_cmd = ["multipass", "exec", node, "--", "bash", "-c", command]
...
"""
    ...
    def setup_local_kubectl(self) -> bool:
        ...
        kubeconfig_path = os.path.join(kube_dir, f"config-{self.config['cluster_name']}")

        # Vérifier que admin.conf existe sur le master
        if platform.system() == "Windows":
            check_admin_conf_cmd = "if (Test-Path /etc/kubernetes/admin.conf) { Write-Output 'File exists' }"
        else:
        ...
        try:
            ...
            if platform.system() == "Windows":
                print(f"Pour utiliser ce cluster: $env:KUBECONFIG='{kubeconfig_path}'")
            else:
            ...
            try:
                ...
                if platform.system() == "Windows":
                    print(f"   $env:KUBECONFIG='{kubeconfig_path}'")
                else:
    ...
    def run(self) -> None:
        ...
        if self.config["rancher_enabled"]:
            ...
            try:
                ...
                # Instructions pour le fichier hosts selon la plateforme
                if platform.system() == "Windows":
                    print(f"  Pour y accéder localement, ajoutez cette entrée dans votre fichier C:\\Windows\\System32\\drivers\\etc\\hosts:")
                else:
                    print(f"  Pour y accéder localement, ajoutez cette entrée dans votre fichier /etc/hosts:")

                print(f"  {master_ip} {master_name}.sslip.io")
            except (subprocess.CalledProcessError, json.JSONDecodeError, KeyError) as e:
                print(f"  Pour y accéder localement, ajoutez l'IP du master dans votre fichier hosts")
